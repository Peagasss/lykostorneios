...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!dbUrl) {
      return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: 'Postgres database connection string not configured.' }) };
    }

    // Silent schema initialization if database is empty
    try {
    ...
    console.error('[Netlify Data Function Error]:', err);
